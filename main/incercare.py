...
def findMove(image, circles, tictactoe_game):
    ...
    for (x, y, r) in circles:
        ...
        for move in tictactoe_game.available_moves():
...
def nextMove(opponentMove, tictactoe_game):
    # Returns the next best move for the arm

    player = 'O'
    # save the opponent's move in the list
    tictactoe_game.make_move(opponentMove // 3, opponentMove % 3, player)
    ...
    computerMove = determine(tictactoe_game, player)

    # save the computer's move in the list
    tictactoe_game.make_move(computerMove // 3, computerMove % 3, player)
    # send_gcode_file(port, computerMove)
    print("Computer Move: ", computerMove + 1)
    tictactoe_game.print_board()

    # play a beep sound the acknowledge the opponent's move
    pygame.init()
...
def drawComputerMoves(image, tictactoe_game):
    ...
    for move in tictactoe_game.available_moves():
        ...
        pt1 = (x1, y1)
        pt2 = (x2, y2)
        cv2.rectangle(image, pt1, pt2, (255, 0, 0), 10)
...
def main():
    ...
    while not tictactoe_game.check_winner('X') and not tictactoe_game.check_winner('O') and not tictactoe_game.is_full():
        ...
        if circles is not None:
            ...
            # get opponent's move
            opponentMove = findMove(image, circles, tictactoe_game)
            if not opponentMove in tictactoe_game.available_moves():
                continue

            # calculate what's the next move
            computerMove = nextMove(opponentMove, tictactoe_game)

            # end the game if there's any winner
            if tictactoe_game.check_winner('X') or tictactoe_game.check_winner('O') or tictactoe_game.is_full():
                break

        # draw opponent's and computer's move on the screen
        drawComputerMoves(image, tictactoe_game)
        drawOpponentMoves(image, tictactoe_game)
        drawRegions(image)
    ...
    print("winner is", tictactoe_game.check_winner('X'), tictactoe_game.check_winner('O'))
...
if __name__ == "__main__":
    regions = regions.Regions(50, 450, 450, 50, 3, 3)
